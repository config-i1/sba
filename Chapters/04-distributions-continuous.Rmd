...
<!-- There are several probability distributions that will be helpful in the further chapters of this textbook. They becomes useful in a variety of contexts, and it is important to be at least aware of them. In this chapter we discuss them in more detail, introducing mathematical notations and providing examples of what these distributions arise from. -->

After discussing the discrete probability distributions, we can now move to the continuous ones. The idea behind them is similar to the one discussed in Chapter \@ref(countDistributions). The main difference arises from the idea discussed in Section \@ref(whatIsRandomVariable): the probability that a continuous random variable will take a specific value is zero. Because of that we should be discussing the probability of a random variable taking a value in an interval. Figure \@ref(fig:distributionContinuousExample) demonstrates an empirical distribution of continuous random variable.
...
plot(seq(-3,3,0.1),pnorm(seq(-3,3,0.1)),type="l",ylab="CDF",xlab="y",main="")
```
...
plot(seq(-3,3,0.1),dnorm(seq(-3,3,0.1)),type="l",ylab="Density",xlab="y",main="")
polygon(c(seq(-3,0,0.01),rev(seq(-3,0,0.01))),
        c(dnorm(seq(-3,0,0.01)), rep(0,length(seq(-3,0,0.01)))), col="grey")
abline(v=0,col="darkred",lwd=2,lty=2)

plot(seq(-3,3,0.1),pnorm(seq(-3,3,0.1)),type="l",ylab="CDF",xlab="y",main="")
lines(c(0,0),c(0,0.5),col="darkred",lwd=2,lty=2)
lines(c(-4,0),c(0.5,0.5),col="darkred",lwd=2,lty=2)
```
...
In this Chapter, we will discuss several continuous distributions, including: Uniform, Normal, Exponential, Laplace, S, Generalised Normal, Asymmetric Laplace, Log Normal, Inverse Gaussian and Gamma.
