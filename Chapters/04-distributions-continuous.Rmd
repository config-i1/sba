...
1. Uniform in Section \@ref(distributionsUniformContinuous);
2. Normal in Section \@ref(distributionsNormal);
3. Log-Normal, which is directly connected to (2) in Section \@ref(distributionLogNormal);
4. Exponential, in Section \@ref(distributionsExponential).

While there exist many more distributions, we cannot discuss all of them and focus only on the most important ones.

Another important topic that we will discuss in this chapter is the convolution of distributions (when several random variables are added up). Understanding the idea behind this is essential in understanding the material from Chapter \@ref(PopulationSampling).
...
One thing that stands out in the figure in comparison with the ones we saw in Chapter \@ref(countDistributions) is that there are no gaps between the bars. In case of the discrete distributions, the bars demonstrated that the variable can only take specific distinct values. In this case, each bar now represents the interval. For example, based on Figure \@ref(fig:distributionContinuousExample), we can say that the probability of obtaining the value between 100 and 105 is higher than for the variable to get any other interval. It also looks like the variable is continuous on the interval between 75 and 125, and we do not observe any values outside of this interval.
...
polygon(c(seq(-5,5,0.1),rev(seq(-5,5,0.1))),
        c(dnorm(seq(-5,5,0.1)), rep(0,length(seq(-5,5,0.1)))), col=17)
```
...
polygon(c(seq(-3,-1,0.01),rev(seq(-3,-1,0.01))),
        c(dnorm(seq(-3,-1,0.01)), rep(0,length(seq(-3,-1,0.01)))), col=15)
lines(c(-1,-1),c(0,dnorm(-1)),col=2,lwd=2,lty=1)

plot(seq(-3,3,0.1),pnorm(seq(-3,3,0.1)),type="l",ylab="Probability",xlab="y",main="")
lines(c(-1,-1),c(0,pnorm(-1)),col=2,lwd=2,lty=2)
lines(c(-4,-1),c(pnorm(-1),pnorm(-1)),col=2,lwd=2,lty=2)
```
...
lines(c(0.25,0.25),c(-3,qnorm(0.25)),col=2,lwd=2,lty=2)
lines(c(-1,0.25),c(qnorm(0.25),qnorm(0.25)),col=2,lwd=2,lty=2)
```
...
\begin{equation}
    f(y, a, b) = \left\{\begin{aligned}
                        & \frac{1}{b-a} & \text{ for } y \in [a, b] \\
                        & 0 & \text{ otherwise }
                    \end{aligned} \right. ,
    (\#eq:ContinuousUniformPDF)
\end{equation}
or in a short form $y \sim \mathcal{U}(a,b)$. It can be represented visually as shown in Figure \@ref(fig:uniformPDF).
...
polygon(c(seq(0,10,0.1),rev(seq(0,10,0.1))),
        c(dunif(seq(0,10,0.1),0,10), rep(0,length(seq(0,10,0.1)))), col=17)
lines(c(0,0), c(0,0.1), lwd=2, col=2)
lines(c(10,10), c(0,0.1), lwd=2, col=2)
```
...
polygon(c(seq(-15,55,0.1),rev(seq(-15,55,0.1))),
        c(dnorm(seq(-15,55,0.1),20,10), rep(0,length(seq(-15,55,0.1)))), col=17)
polygon(c(seq(20,30,0.1),rev(seq(20,30,0.1))),
        c(dnorm(seq(20,30,0.1),20,10), rep(0,length(seq(20,30,0.1)))), col=10)
lines(c(20,20),c(0,dnorm(20,20,10)), lwd=2, col=2)
lines(c(30,30),c(0,dnorm(30,20,10)), lwd=2, col=2)
...
plot(test, xlim=c(30,70), main="", freq=FALSE, ylab="Probability", xlab="y", col=17)
lines(seq(30,70,0.1),dnorm(seq(30,70,0.1),50,5), col=3, lwd=2)
```
...
plot(test, xlim=c(30,70),
     main="", freq=FALSE, ylab="Probability", xlab="y", col=17)
polygon(c(seq(40.5,41.5,0.1),rev(seq(40.5,41.5,0.1))),
        c(dnorm(seq(40.5,41.5,0.1),50,5), rep(0,length(seq(40.5,41.5,0.1)))), col=9)
lines(seq(30,70,0.1),dnorm(seq(30,70,0.1),50,5), col=3, lwd=2)
lines(c(40.5,40.5),c(0,dnorm(40.5,50,5)), lwd=2, col=2)
lines(c(41.5,41.5),c(0,dnorm(41.5,50,5)), lwd=2, col=2)
#Second plot
...
polygon(c(seq(-1.5,0.5,0.01),rev(seq(-1.5,0.5,0.01))),
        ...
        col=10)
lines(rep(0.5,2), c(0,dnorm(0.5,0,0.5)-0.01), lty=1, lwd=2, col=0)
...
lines(rep(0.5,2), c(5,exp(0.5)), lty=2, lwd=2, col=2)
lines(c(0.5,2), rep(exp(0.5),2), lty=2, lwd=2, col=2)
...
polygon(c(dlnorm(exp(seq(-1.5,0.5,0.01)),0,0.5),rep(0,length(seq(-1.5,0.5,0.01)))),
        ...
        col=10)
abline(h=exp(0.5), lty=1, lwd=2, col=0)
abline(h=exp(0.5), lty=2, lwd=2, col=2)
```
...
plot(seq(0,5,0.01), dlnorm(seq(0,5,0.01),0,0.1), type="l",
     ...
     xlim=c(0,4))
lines(seq(0,5,0.01), dlnorm(seq(0,5,0.01),0,0.5), col=2, lwd=2)
lines(seq(0,5,0.01), dlnorm(seq(0,5,0.01),0.5,0.1), col=3, lwd=2)
lines(seq(0,5,0.01), dlnorm(seq(0,5,0.01),0.5,0.5), col=4, lwd=2)
...
plot(seq(0,4,0.1), dexp(seq(0,4,0.1),2), type="l",
     ...
     xlim=c(0,3))
lines(seq(0,4,0.1), dexp(seq(0,4,0.1),1), col=2, lwd=2)
lines(seq(0,4,0.1), dexp(seq(0,4,0.1),0.5), col=3, lwd=2)
...
plot(seq(0,4,0.1), pexp(seq(0,4,0.1),2), type="l",
     ...
     xlim=c(0,3))
lines(seq(0,4,0.1), pexp(seq(0,4,0.1),1), col=2, lwd=2)
lines(seq(0,4,0.1), pexp(seq(0,4,0.1),0.5), col=3, lwd=2)
legend("topright", legend=c(TeX("$\\lambda=2$"),TeX("$\\lambda=1$"),TeX("$\\lambda=0.5$")),
       lwd=2, col=c(1:3))
```
...
# 1.5
lines(c(1.5,1.5),c(-5,log(1-pexp(1.5,2))), lty=2, lwd=2, col=15)
lines(c(-1,1.5),log(rep(1-pexp(1.5,2),2)), lty=2, lwd=2, col=15)
# 0.5
lines(c(0.5,0.5),c(-5,log(1-pexp(0.5,2))), lty=2, lwd=2, col=15)
lines(c(-1,0.5),log(rep(1-pexp(0.5,2),2)), lty=2, lwd=2, col=15)
# 1
lines(c(1,1),c(-5,log(1-pexp(1,2))), lty=2, lwd=2, col=15)
lines(c(-1,1),log(rep(1-pexp(1,2),2)), lty=2, lwd=2, col=15)
```
...
Let us record the value on one dice as $x$, and the value on the second one as $y$. Rolling both of them, means that we need to add $x$ to $y$ to get a new score $z$. For example, we had 3 on one dice and 5 on the other one, which means that $x=3$, $y=5$ and $z=x+y=3+5=8$. The span of values of $z$ is now between 2 and 12, because the lowest score for each die is 1 (thus $1+1=2$), while the highest one is 6 (6+6=12).
...
```{r uniformPMFRepeated, fig.cap="Probability Mass Function of Uniform distribution for 1d6.", fig.height=3, fig.width=8, echo=FALSE}
par(mar=c(4,4,1,1))
test <- barplot(rep(1/6,6), ylim=c(0,0.2),
                ylab="Probability",xlab="x", col=10)
axis(side=1,at=test,labels=c(1:6), col.axis=4)
```
...
One way of looking at this process, is to first acknowledge that $y=z-x$ and then by sliding one distribution above the other by changing the value of $x$. For example, to have $z=3$ we can have $x=1$ and $y=3-1=2$, or $x=2$ and $y=3-2=1$. This corresponds to the sliding shown in Figure \@ref(fig:uniformPMFSlide01), where the red distribution is placed above the blue one and in this specific example, we get an intersection of two bars, each of which gives us the same $z=3$ (which is shown on the right-hand side plot). The probabilities of each bar are $\frac{1}{6}$, the intersection gives us the multiplication of those probabilities (thus $\frac{1}{36}$) and we have two bars that have these intersections, so the final answer is the same as we obtained above: $\frac{1}{36} + \frac{1}{36} = \frac{1}{18}$. The barplot on the right-hand side of the image shows how specific probabilities appear. The height of the bars corresponds to the probability of the specific $z$ score.

```{r uniformPMFSlide01, fig.cap="Convolution of two PMFs for 1d6, giving the join score of 3.", fig.height=3, fig.width=8, echo=FALSE}
x <- c(1:6)
...
test2 <- barplot(y/6,
                 ...
                 add=T)
abline(h=0, lwd=2)
text(test1, 1/6, labels=c(6:1), pos=3, col=2)
text(test2[-c(1:4)], 0, labels=c(1:6), pos=1, col=4)
text(test1[c(5:6)], 1/12, labels=rep(3,2), col=1)

# Overall one
apply(expand.grid(1:6,1:6), 1, sum) |> table() -> z
par(mar=c(0,1,0,0))
test3 <- barplot(z[1]/sum(z),
                 ylim=c(-0.02,0.21), xlim=c(0,12),
                 col=9,
                 axes=F, names.arg="")
test3 <- barplot(c(0,z[2]/sum(z)),
                 ylim=c(-0.02,0.21), xlim=c(0,12),
                 col=8,
                 axes=F, names.arg="",
                 add=T)
test4 <- barplot(c(0,z[2]/sum(z)),
                 ylim=c(-0.02,0.21), xlim=c(0,12),
                 col=10,
                 axes=F, names.arg="",
                 add=T)
text(test3, z[1:2]/sum(z), labels=TeX(paste0("$\\frac{",z[1:2],"}{",sum(z),"}$")), pos=3)
text(test3, 0, labels=c(2:12)[1:2], pos=1, col=4)
abline(h=0, lwd=2)
```
...
```{r uniformPMFSlide02, fig.cap="Convolution of two PMFs for 1d6, giving the join score of 7.", fig.height=3, fig.width=8, echo=FALSE}
x <- c(1:6)
y <- table(12-x+1)
...
abline(h=0, lwd=2)
text(test1, 1/6, labels=c(6:1), pos=3, col=2)
text(test2, 0, labels=c(1:6), pos=1, col=4)
text(test1[c(1:6)], 1/12, labels=rep(7,6), col=1)

# Overall one
apply(expand.grid(1:6,1:6), 1, sum) |> table() -> z
par(mar=c(0,1,0,0))
test3 <- barplot(z[1:5]/sum(z),
                 ylim=c(-0.02,0.21), xlim=c(0,12),
                 col=9,
                 axes=F, names.arg="")
test3 <- barplot(c(rep(0,5),z[6]/sum(z)),
                 ylim=c(-0.02,0.21), xlim=c(0,12),
                 col=8,
                 axes=F, names.arg="",
                 add=T)
test4 <- barplot(c(rep(0,5),z[6]/sum(z)),
                 ylim=c(-0.02,0.21), xlim=c(0,12),
                 col=10,
                 axes=F, names.arg="",
                 add=T)
text(test3, z[1:6]/sum(z), labels=TeX(paste0("$\\frac{",z[1:6],"}{",sum(z),"}$")), pos=3)
abline(h=0, lwd=2)
text(test3, 0, labels=c(2:12)[1:6], pos=1, col=4)
```
...
```{r uniformPMFSlide03, fig.cap="Convolution of two PMFs for 1d6, giving the join score of 9.", fig.height=3, fig.width=8, echo=FALSE}
x <- c(1:6)
...
test1 <- barplot(table(x)/6,
                 ...
                 add=T)
abline(h=0, lwd=2)
text(test2[-c(1:2)], 1/6, labels=c(6:1), pos=3, col=2)
text(test1, 0, labels=c(1:6), pos=1, col=4)
text(test1[c(3:6)], 1/12, labels=rep(9,4), col=1)

# Overall one
apply(expand.grid(1:6,1:6), 1, sum) |> table() -> z
par(mar=c(0,1,0,0))
test3 <- barplot(z[1:7]/sum(z),
                 ylim=c(-0.02,0.21), xlim=c(0,12),
                 col=9,
                 axes=F, names.arg="")
test3 <- barplot(c(rep(0,7),z[8]/sum(z)),
                 ylim=c(-0.02,0.21), xlim=c(0,12),
                 col=8,
                 axes=F, names.arg="",
                 add=T)
test4 <- barplot(c(rep(0,7),z[8]/sum(z)),
                 ylim=c(-0.02,0.21), xlim=c(0,12),
                 col=10,
                 axes=F, names.arg="",
                 add=T)
text(test3, z[1:8]/sum(z), labels=TeX(paste0("$\\frac{",z[1:8],"}{",sum(z),"}$")), pos=3)
abline(h=0, lwd=2)
text(test3, 0, labels=c(2:12)[1:8], pos=1, col=4)
```
...
```{r uniformPMFCombined, fig.cap="Probability Mass Function of the sum of two Uniform distributions for 1d6.", fig.width=8, fig.height=3, echo=FALSE}
apply(expand.grid(1:6,1:6), 1, sum) |> table() -> z
par(mar=c(3,4,0,1))
test <- barplot(z/sum(z),
                ylim=c(0,0.23), ylab="Probability", xlab="", 
                col=9, axes=F, names.arg="")
text(test, z/sum(z), labels=TeX(paste0("$\\frac{",z,"}{",sum(z),"}$")), pos=3)
axis(1, at=test, labels=names(z))
title(xlab="z", line=2.2, cex=1.2)
```
...
for(i in 1:length(mu1)){
    ...
    polygon(c(x,rev(x)),
            c(dnorm(x, mu1[i], 1), rep(0,length(x))), col=8)
    lines(seq(-3,3,0.1),dnorm(seq(-3,3,0.1), mu2))
    polygon(c(x,rev(x)),
            c(dnorm(x, mu2, 1), rep(0,length(x))), col=10)
    ...
    plot(z, dnorm(z, mu2, sqrt(2)),
         ...
         xlim=c(-5,5), ylim=c(0,0.4), axes=F)
    polygon(c(z,rev(z)),
            c(dnorm(z, mu2, sqrt(2)), rep(0,length(z))), col=17)
...
```{r expConvolution, fig.cap="Convolution of two Exponential distributions with $\\lambda=1$.", fig.width=7, fig.height=6, echo=FALSE}
mu2 <- 1
mu1 <- c(1, 3, 5)
x <- seq(0,10,0.01)
...
for(i in 1:length(mu1)){
    ...
    polygon(c(mu1[i]-x,rev(mu1[i]-x)),
            c(dexp(x, mu2), rep(0,length(x))), col=8)
    polygon(c(x,rev(x)),
            c(dexp(x, mu2), rep(0,length(x))), col=10)
    axis(2)
    ...
    polygon(c(z,rev(z)),
            c(dgamma(z, shape=2, rate=1), rep(0,length(z))), col=17)
...
\begin{equation}
    ...
    (\#eq:convolution)
\end{equation}
The sliding that we discussed in the previous subsections, in this formula is done by the element $g(z-x)$, which takes values from the right tail of distribution due to $z-x$, thus reflecting the idea that we can obtain the same value of $z$ with a set of combinations of $x$ and $y$, each having some assigned density value from the functions $f(x)$ and $g(y)$ respectively.

<!-- Let's consider an example of the addition of two random variables following uniform distribution with different parameters: $x \sim \mathcal{U}(1,3)$ and  $y \sim \mathcal{U}(2,4)$. Their PDFs can be written as: -->
<!-- \begin{equation} -->
<!--     f(x, 1, 3) = \left\{\begin{aligned} -->
<!--                     & \frac{1}{3-1} & \text{ for } x \in [1, 3] \\ -->
<!--                     & 0 & \text{ otherwise } -->
<!--                 \end{aligned} \right. , -->
<!--     (\#eq:uniformExampleX) -->
<!-- \end{equation} -->
<!-- and -->
<!-- \begin{equation} -->
<!--     f(y, 2, 4) = \left\{\begin{aligned} -->
<!--                     & \frac{1}{4-2} & \text{ for } y \in [2, 4] \\ -->
<!--                     & 0 & \text{ otherwise } -->
<!--                 \end{aligned} \right.  -->
<!--     (\#eq:uniformExampleY) -->
<!-- \end{equation} -->
<!-- respectively. Based on $z=x+y$, we can rewrite the PDF \@ref(eq:uniformExampleY) into: -->
<!-- \begin{equation} -->
<!--     f(z-x, 2, 4) = \left\{\begin{aligned} -->
<!--                     & \frac{1}{2} & \text{ for } z-x \in [2, 4] \\ -->
<!--                     & 0 & \text{ otherwise } -->
<!--                 \end{aligned} \right. . -->
<!--     (\#eq:uniformExampleYRewritten) -->
<!-- \end{equation} -->
<!-- Given the summation of the two variables, the support for $z$ is in the region $[3, 7]$. For the other values, the PDF of $z$ will be equal to zero. -->

<!-- Inserting \@ref(eq:uniformExampleX) and \@ref(eq:uniformExampleY) in \@ref(eq:convolution), leads to: -->
<!-- \begin{equation} -->
<!--     h(z) = \int_{-infty}^{infty} f(x, 1, 3) f(z-x, 2, 4) dx = . -->
<!--     (\#eq:convolutionExample01) -->
<!-- \end{equation} -->
