...
## Log-Normal distributions {#distributionLogNormal}
Log-Normal distribution is closely related to the Normal one and is supported for the positive values of $y$. It is defined as a distribution arising after a transformation of a variable. It is said that $y \sim \log\mathcal{N}(\mu_y, \sigma^2)$ if $\log y \sim \mathcal{N}(\mu_y, \sigma^2)$. This can also be written differently if we take $x=\log(y)$: $e^x \sim \log\mathcal{N}(\mu_y, \sigma^2)$ if $x \sim \mathcal{N}(\mu_y, \sigma^2)$. Figure \@ref(fig:normalLogConnection) shows the connection between Normal and Log-Normal distributions. In that plot, we can see how because of the $y=e^x$ transformation the density changes.
...
The PDF of the Log-Normal distribution is typically skewed with a long right tail. The PDF itself is:
\begin{equation}
