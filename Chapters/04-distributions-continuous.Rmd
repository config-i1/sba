...
We have already touched upon the Exponential distribution, when we discussed the arrival times (Section \@ref(distributionPoisson)). We mentioned that if a process is memoryless, then the following holds:
\begin{equation*}
    \mathrm{P}(y > \tau_1 + \tau_2) = \mathrm{P}(y > \tau_1)\mathrm{P}(y > \tau_2) .
\end{equation*}
Exponential distribution is typically used for modelling arrival time, because it relies on this property. Here is its PDF:
\begin{equation}
    f(y, \lambda) = \lambda e^{-\lambda y} ,
    (\#eq:ExponentialPMF)
\end{equation}
where $y$ is a positive number and $\lambda$ is the rate parameter. This PDF is shown in Figure \@ref(fig:dexpPlot).
...
The plot in Figure \@ref(fig:dexpPlot) shows that there is more likely for the variable $y$ to get lower values (closer to zero) than the higher ones. The parameter $\lambda$ controls the steepness of decline of the density curve with increase of $y$: the higher the rate is, the more likely it is that the event will occur earlier and less likely that it will occur later.
...
plot(seq(0,4,0.1), pexp(seq(0,4,0.1),2), type="l",
     ylab="Probability", xlab="y", main="", lwd=2,
     xlim=c(0,3))
...
\begin{equation}
    \mathrm{F}(y, \lambda) = 1 - e^{- \lambda y}.
    (\#eq:ExponentialCDF)
\end{equation}
...
\begin{equation*}
    \mathrm{F}(y < 1.5, 2) = 1 - e^{- 2 \times 1.5} \approx 0.95 .
\end{equation*}
...
\begin{equation*}
    \mathrm{P}(y > 1.5) = \mathrm{P}(y > 0.5) \mathrm{P}(y > 1)
\end{equation*}
and $\lambda=2$. Note that $P(y>a)=1-F(a)$ by definition of CDF. In terms of probabilities of Exponential distribution, this means that:
\begin{equation*}
    \mathrm{P}(y > 1.5) = (1-\mathrm{F}(1, 2)) (1-\mathrm{F}(0.5, 2)) .
\end{equation*}
...
\begin{equation}
    \log \left( \mathrm{P}(y > 1.5) \right) = \log \left(1-\mathrm{F}(1, 2) \right) + \log \left( 1-\mathrm{F}(0.5, 2)  \right) .
...
\begin{equation}
    \log \left( \mathrm{P}(y > 1.5) \right) = \log \left(1- 1 + e^{- 2 \times 1} \right) + \log \left(1- 1 + e^{- 2 \times 0.5} \right) = -2 - 1 = -3 ,
    (\#eq:ExponentialCDFMemory02)
\end{equation}
which is obtained independently for $\log(1-F(1.5, 2))$
We can also plot $\log(1-F(y, 2))$ in Figure \@ref(fig:pexpPlotMemory) to show that \@ref(eq:ExponentialCDFMemory02) holds, when the values on y-axis are added.
...
plot(seq(0,4,0.1), log(1-pexp(seq(0,4,0.1),2)), type="l",
     ylab="log(1-F(y,2))", xlab="y", main="", lwd=2,
...
Note that because of the logarithm of the CDF, the function has now become linear, and the property becomes more apparent. Accidentally, $-\log(1-F(y, \lambda))$ is called cumulative hazard function.

<!-- Expectation, Variance, how to use with Poisson -->
