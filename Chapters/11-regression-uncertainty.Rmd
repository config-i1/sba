...
\begin{equation}
    \beta_j \in (b_j + t_{\alpha/2}(T-k) s_{b_j}, b_j + t_{1-\alpha/2}(T-k) s_{b_j}),
    (\#eq:MLRcovarianceMatrix)
\end{equation}
where $s_{b_j}$ is the standard error of the parameter $b_j$. All modern software does all these calculations automatically, so we do not need to do them manually. Here is an example:
...
par(mfcol=c(1,2))
plot(cars$speed, cars$dist,
     xlab="Speed", ylab="Stopping Distance", main=TeX("H$_0$: $\\beta_j = 0$"))
abline(h=mean(cars$dist),col="red")
plot(cars$speed, cars$dist,
     xlab="Speed", ylab="Stopping Distance", main=TeX("H$_1$: $\\beta_j \\neq 0$"))
