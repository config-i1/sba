...
::: warning
Linear algebra is used below! Tread lightly!
:::
...
\begin{equation}
    ...
    (\#eq:MLROLSVariance05)
\end{equation}
The second element in the brackets can be rewritten as $\left( \left( {\mathbf{X}}^\prime {\mathbf{X}} \right)^{-1} {\mathbf{X}}^\prime {\boldsymbol{\epsilon}} \right)^\prime = {\boldsymbol{\epsilon}} {\mathbf{X}} \left( {\mathbf{X}}^\prime {\mathbf{X}} \right)^{-1} $. Substituting this to \@ref(eq:MLROLSVariance05) and moving explanatory variables from the covariance leads to:
\begin{equation}
...
\begin{equation}
    \mathrm{V}\left( {\boldsymbol{b}} \right) = \left( {\mathbf{X}}^\prime {\mathbf{X}} \right)^{-1} \hat{\sigma}^2 ,
    (\#eq:MLROLSVarianceFinal)
\end{equation}
where $\hat{\sigma}^2 = \frac{1}{n-k} \sum_{j=1}^n e_j^2$ is the estimate of the true variance based on the applied model.
:::
