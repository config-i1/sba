...
To test the hypothesis about a parameter in regression, we need to follow the procedure, described in Section \@ref(hypothesisTesting). First step is to formulate the null and alternative hypotheses. As discussed earlier in this section, the conventional way of doing that is by checking whether the true value of parameter is zero in the population:
\begin{equation*}
    \begin{aligned}
        \mathrm{H}_0: \beta_i = 0 \\
        \mathrm{H}_1: \beta_i \neq 0
...
Then, we select the significance level. The typical one is 5%, but we can choose another one based on the specific situation.

After that, we need to select the test to use. As discussed earlier in this chapter, if we can assume that the Central Limit Theorem holds (typically, for the linear regression estimated using the OLS, it does), we can use the Normal distribution. But given that the standard deviations of parameters are not known, and are estimated, we need to use Student's t distribution. The formula for calculating the t-statistics is funamentally the same as in Section \@ref(hypothesisTestingBasics):
\begin{equation}
    t = \frac{b_i - 0}{s_{b_i}} ,
    (\#eq:ttestFormulaReg)
\end{equation}
where $b_i$ is the estimate of the parameter $\beta_i$ and $s_{b_i}$ is the standard error of the parameter.
