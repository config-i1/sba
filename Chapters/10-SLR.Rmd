...
OLS applied to any model guarantees two important properties:

1. $\frac{1}{n} \sum_{j=1}^n e_j = 0$,
2. $\frac{1}{n} \sum_{j=1}^n e_j x_{i,j} = 0$ for any $i$.
...
\begin{equation}
    \begin{aligned}
        \sum_{j=1}^n e_j x_j = & \sum_{j=1}^n y_j x_j - \frac{1}{n}\sum_{j=1}^n y_j \sum_{j=1}^n x_j + \frac{n \sum_{j=1}^n y_j x_j - \sum_{j=1}^n y_j \sum_{j=1}^n x_j}{n \sum_{j=1}^n x_j^2 - \left(\sum_{j=1}^n x_j \right)^2} \left( \frac{1}{n}\sum_{j=1}^n x_j \sum_{j=1}^n x_j - \sum_{j=1}^n x_j^2 \right) .
    \end{aligned}
    (\#eq:sumOfResiduals06)
\end{equation}
...
In order to see that the second property implies that the correlation between the residuals and regressors is equal to zero, we need to take a step back and consider the covariance between $e_j$ and $x_j$:
\begin{equation}
...
\begin{equation}
    \mathrm{cov}(e_j,x_j) = \sum_{j=1}^n e_j (x_j - \bar{x}) = \sum_{j=1}^n e_j x_j - \bar{x} \sum_{j=1}^n e_j .
    (\#eq:sumOfResiduals10)
\end{equation}
The second sum in \@ref(eq:sumOfResiduals10) is equal to zero due to the same property, so we see that in this special case the covariance between the residuals and the regressor is equal to $\sum_{j=1}^n e_j x_j$.
