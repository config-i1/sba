...
Imagine that you have a situation when you sell a luxury expensive product and you track its sales. On Monday nobody came to buy it. On Tuesday, you had a client who bought it once. On Wednesday, Thursday and Friday nobody bought it again. But on Saturday two different people came and bought one each. Can we somehow model this process and maybe forecast how many products we can sell over the next week?

Yes. The Negative Binomial distribution is one of the appropriate tools here. It models the number of failures in an experiment (e.g. no sales) before some defined number of successes occurs (sale). In contrast with the Binomial distribution, in the NegBin, the number of the total trials is unknown. And in the example above, we could say that a success is when someone buys our product, and we could imagine that people are thinking of buying, they toss an imaginary coin and either do or do not make this decision based on some probability.
...
Figure \@ref(fig:NegBinPMF) shows that, for example, if p=0.2, and we want to have one success (k=1), we have the probability of 0.2 that we will succeed without having any failures. To make this easier to understand, if we have a 1d5 (five-sided dice), and we define success as rolling 5 on it, the probability that we will have it is 0.2 (this is one out of five possible outcomes), which corresponds to the first bar in the first plot in Figure \@ref(fig:NegBinPMF). The probability that we will roll 5 in the second trial is $0.2 \times (1-0.2) = 0.16$, which corresponds to the second bar in the first plot in Figure \@ref(fig:NegBinPMF), etc. With $k=2$, we model a situation when we need to get five two times in an experiment (not necessarily sequentially).

The PMF of this distribution is written as:
\begin{equation}
