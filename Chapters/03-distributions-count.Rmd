...
From the example above, we can take away several important characteristics of Binomial distribution:

1. In each experiment, there are only two outcomes: buy or do not buy;
2. The probability of success (someone decides to buy a machine) is fixed between the experiments: we assume that the probability that a hospital A decides to buy a machine is the same as for the hospital B;
3. The experiments are independent: if one hospital buys a machine, this should not impact the decision of another one;
4. The number of experiments $n$ is limited and known.



What if we have more trials?


## Rolling a dice -- Uniform distribution {#distributionUniform}
...
<!-- Add a note on multiple dice rolls -- Multinomial distribution??? -->
