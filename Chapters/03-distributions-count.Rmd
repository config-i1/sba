...
::: solution
1. This is an example of Bernoulli distribution: it has two outcomes and a probability of success.

2. We will encode the miss as zero and the score as one. Based on that, taking the mean of the outcomes, we can estimate the mean of Bernoulli probability of miss:
\begin{equation*}
    \bar{y} = \hat{p} = \frac{36}{100} = 0.36.
\end{equation*}
So, when Vasiliy throws the next ball in the bin, he will miss with the probability of 0.36.

3. The variance is $p \times (1-p) = 0.36 \times 0.64 = 0.2304$.
:::
