...
y <- c(1:6,5:1)/36
names(y) <- c(2:12)
yBarplot <- barplot(y, xlab="Dice score", ylab="Probability of outcome", col=17)
...
yBarplot <- barplot(cumsum(y), xlab="Cumulative dice score", ylab="Probability of outcome", col=17)
axis(1, at=yBarplot, labels=FALSE)
```
...
## Expected value and variance of a random variable {#rvExpectationAndVariance}
...
1. the expected value, which describes the distribution's centre, and
2. the variance, which describes its spread.

We begin with a practical example to build an intuitive understanding before moving to more formal definitions.


### Daily Car Sales at DiCarlo Motors
...
### Calculating the Expected Value (The Mean)
...
\begin{equation*}
    \begin{aligned}
        \mu = & (0 \times 0.18) + (1 \times 0.39) + (2 \times 0.24) + (3 \times 0.14) + (4 \times 0.04) + (5 \times 0.01) = \\
              & 0 + 0.39 + 0.48 + 0.42 + 0.16 + 0.05 = 1.5 .
    \end{aligned}
\end{equation*}

So, the expected value is 1.5 cars per day. While it is impossible to sell exactly 1.5 cars on any single day, this number tells us something else quite important: that over a long period of time, DiCarlo Motors can anticipate selling on average 1.5 cars per day. In each single day, the specific sales would vary according to the probabilities in the previous Subsection, but if we take all the sales and divide them by the number of days, we would get 1.5 cars per day. This measure is fundamental for forecasting; for example, the company could project average monthly sales of 45 cars (1.5 cars/day \times 30 days). This could be used for further decision making.

The mathematical formula for the expected value of a discrete random variable is:
\begin{equation}
    \mu = P(x_1) x_1 + P(x_2) x_2 + \dots + P(x_n) x_n =  \sum_{j=1}^n P(x_i) x_i ,
    (\#eq:expectationDiscrete)
\end{equation}
where $x_i$ is the value of interest (e.g. number of cars) and $P(x_i)$ is the probability of this specific outcome.
...
Using our calculated mean of $\mu = 1.5$, we can find the variance for the car sales data:
\begin{equation*}
    \begin{aligned}
        \sigma^2 = & (0 − 1.5)^2 \times 0.18 + (1 − 1.5)^2 \times 0.39 + (2 − 1.5)^2 \times 0.24 + (3 − 1.5)^2 \times 0.14 + (4 − 1.5)^2 \times 0.04 + (5 − 1.5)^2 \times 0.01 = \\
        & 0.405 + 0.0975 + 0.06 + 0.315 + 0.25 + 0.1225 = 1.25
    \end{aligned}
\end{equation*}

So, the variance in our example is 1.25, but its units are "cars squared" (because we had squares in the formula above), which is not intuitive. To get a measure of spread in the original units (cars), we can calculate the standard deviation ($\sigma$), which is simply the positive square root of the variance ($\sigma = \sqrt{\sigma^2}$:
\begin{equation*}
...
Mathematically, the formula of variance is written as:
\begin{equation}
    \mu = P(x_1) (x_1 - \mu)^2 + P(x_2) (x_2 - \mu)^2 + \dots + P(x_n) (x_n - \mu)^2 =  \sum_{j=1}^n P(x_i) (x_n - \mu)^2 ,
    (\#eq:varianceDiscrete)
\end{equation}
...
--------------------------------------------------------------------------------
2. Formal Definitions and Core Concepts
With the practical calculations from our car sales example as a foundation, we can now formalise the definitions of expected value and variance. These two measures are the cornerstones for summarising the key characteristics of any probability distribution, providing a snapshot of its central tendency and dispersion.
2.1. The Expected Value (\mu)
The Expected Value (or mean) of a discrete random variable X is formally defined as the weighted average of all its possible values. The weight assigned to each possible value, 𝑥ᵢ, is its corresponding probability of occurrence, P(𝑥ᵢ).
This method differs fundamentally from calculating a simple population mean for raw data. A simple mean gives equal weight to every single observation, as shown in the formula \mu = (Σ𝑥) / N, where each value is effectively weighted by 1/N. In contrast, the expected value \mu = Σ 𝑥ᵢ * 𝑃(𝑥ᵢ) weights each distinct outcome according to its probability. This provides a more accurate and meaningful measure of the distribution's true centre, reflecting the likelihood of each outcome. In the DiCarlo Motors example, calculating a simple mean of the six possible outcomes (0, 1, 2, 3, 4, 5) would yield 2.5, a misleading figure that ignores the fact that selling one car (P=0.39) is far more likely than selling five (P=0.01). The expected value of 1.5 correctly accounts for these probabilities.
2.2. The Variance (σ^2 ) and Standard Deviation (σ)
The Variance of a discrete random variable X is formally defined as the weighted average of the squared deviations from the mean, (𝑥ᵢ − 𝜇)^2 . Its purpose is to quantify the dispersion or variability of the possible outcomes around the expected value. A distribution with outcomes that are typically far from the mean will have a high variance, indicating a wide spread. Conversely, a distribution with outcomes clustered tightly around the mean will have a low variance.
The Standard Deviation (σ) is the positive square root of the variance. Its primary advantage is that it measures dispersion in the same units as the original random variable. While the variance of car sales was 1.25 "cars squared," the standard deviation of 1.118 "cars" is a much more intuitive and directly interpretable measure of spread.
Ultimately, these two measures work in tandem: the expected value pinpoints the centre of gravity for the distribution, while the standard deviation quantifies the typical distance of an outcome from that centre.
...
barplot(ylim=c(0,1),
        ylab="Probability",xlab="y", col=17, main=TeX("$\\lambda=0.1$"))
dpois(c(0:10),lambda=0.5) |> setNames(0:10) |>
barplot(ylim=c(0,1),
        ylab="Probability",xlab="y", col=17, main=TeX("$\\lambda=0.5$"))
dpois(c(0:10),lambda=1) |> setNames(0:10) |>
barplot(ylim=c(0,1),
        ylab="Probability",xlab="y", col=17, main=TeX("$\\lambda=1$"))

dpois(c(0:10),lambda=2) |> setNames(0:10) |>
barplot(ylim=c(0,1),
        ylab="Probability",xlab="y", col=17, main=TeX("$\\lambda=2$"))
dpois(c(0:10),lambda=3) |> setNames(0:10) |>
barplot(ylim=c(0,1),
        ylab="Probability",xlab="y", col=17, main=TeX("$\\lambda=3$"))
dpois(c(0:22),lambda=10) |> setNames(0:22) |>
barplot(ylab="Probability", xlab="y", col=17, main=TeX("$\\lambda=10$"))
```
...
test <- barplot(rep(1/6,6), ylim=c(0,1),
                ylab="Probability",xlab="y", col=17)
