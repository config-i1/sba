...
\begin{equation}
    ...
    (\#eq:SSTProof03)
\end{equation}
The second sum in \@ref(eq:SSTProof03) is equal to zero, because OLS guarantees that the in-sample mean of error term is equal to zero. The second one can be expanded to:
\begin{equation}
    \begin{aligned}
    ...
    (\#eq:SSTProof04)
\end{equation}
We see the sum of errors in the first sum of \@ref(eq:SSTProof04) again, so the first elements is equal to zero again. The second term is equal to zero as well due to OLS estimation (this will be discussed in Chapter \@ref(assumptions)) because it guarantees that $\sum_{j=1}^n e_j x_j = 0$. This means that:
\begin{equation}
...
Building upon thata, there is a measure called "Coefficient of Determination", which is calculated based on the sums of squares discussed above:
\begin{equation}
