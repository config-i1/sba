...
# Set up a blank plot
plot(1, type="n", xlim=c(3,5), ylim=c(1,7), axes=FALSE, xlab="", ylab="", asp=1)
...
```{r treeForVampire, fig.height=6, fig.width=10, echo=FALSE, fig.cap="Tree diagram for the vampire example."}
# Set up blank plot
...
First, we plot the tree diagram to better understand what we have and what we are missing. Here are the notations we use in that figure:

- "Spam" event is denoted with "S",
- "Not Spam" is "NS",
- The word "discount" is in the text - "D",
- The word "discount" is not in the text - "ND".

```{r treeForSpam01, fig.height=6, fig.width=10, echo=FALSE, fig.cap="Tree diagram for the spam detection example."}
# Set up blank plot
plot(1, type="n", xlim=c(0,10), ylim=c(1,10), axes=FALSE, xlab="", ylab="")
...
text(3.1, 10, "The email is:", cex=1.3, col=1)
text(3.1, 8, "Spam", cex=1.1, col=3)
text(3.1, 4, "Not spam", cex=1.1, col=4)

text(1.9, 7, TeX("$P(S) = 0.4$"), cex=1.1, col=3)
text(1.9, 3, TeX("$P(NS) = 0.6$"), cex=1.1, col=4)
...
text(6, 10, "The word detection:", cex=1.3, col=1)

points(6, 8, pch=21, bg=7, cex=2)
text(6, 8, "Discount", cex=1.1, col=7, pos=4)
points(6, 6, pch=21, bg=6, cex=2)
text(6, 6, "No discount", cex=1.1, col=6, pos=4)
points(6, 4, pch=21, bg=7, cex=2)
text(6, 4, "Discount", cex=1.1, col=7, pos=4)
points(6, 2, pch=21, bg=6, cex=2)
text(6, 2, "No discount", cex=1.1, col=6, pos=4)

# Probabilities
text(4.5, 8.5, TeX("$P(D | S) = 0.7$"), cex=1.1, col=7)
text(5, 5.5, TeX("$P(ND | S) = 0.3$"), cex=1.1, col=6)
text(4.5, 4.4, TeX("$P(D | NS) = 0.1$"), cex=1.1, col=7)
text(5, 1.5, TeX("$P(ND | NS) = 0.9$"), cex=1.1, col=6)
```

As we see in the Tree diagram in Figure \@ref(fig:treeForSpam01), we know all the probabilities for all the outcomes. So, what is the problem then? The problem is that we need to invert it to get the answer to our question, i.e. build a new tree where the word "discount" is the first vertex (Figure \@ref(fig:treeForSpam02)).

```{r treeForSpam02, fig.height=6, fig.width=10, echo=FALSE, fig.cap="Tree diagram for the spam detection example, starting from the word 'discount'."}
# Set up blank plot
plot(1, type="n", xlim=c(0,10), ylim=c(1,10), axes=FALSE, xlab="", ylab="")
...
segments(1, 5, 3, 7, lwd=2)  # p1 - upper path
segments(1, 5, 3, 3, lwd=2)  # p2 - lower path
points(3, 7, pch=21, bg=7, cex=2)
points(3, 3, pch=21, bg=6, cex=2)

text(3.1, 10, "The word detection:", cex=1.3, col=1)
text(3.1, 8, "Discount", cex=1.1, col=7)
text(3.1, 4, "No discount", cex=1.1, col=6)

text(1.9, 7, TeX("$P(D) = ?$"), cex=1.1, col=7)
text(1.9, 3, TeX("$P(ND) = ?$"), cex=1.1, col=6)
...
text(6, 10, "The email is:", cex=1.3, col=1)

points(6, 8, pch=21, bg=3, cex=2)
text(6, 8, "Spam", cex=1.1, col=3, pos=4)
points(6, 6, pch=21, bg=4, cex=2)
text(6, 6, "Not spam", cex=1.1, col=4, pos=4)
points(6, 4, pch=21, bg=3, cex=2)
text(6, 4, "Spam", cex=1.1, col=3, pos=4)
points(6, 2, pch=21, bg=4, cex=2)
text(6, 2, "Not spam", cex=1.1, col=4, pos=4)

# Probabilities
text(4.5, 8.5, TeX("$P(S | D) = ?$"), cex=1.1, col=3)
text(5, 5.5, TeX("$P(NS | D) = ?$"), cex=1.1, col=4)
text(4.5, 4.4, TeX("$P(S | ND) = ?$"), cex=1.1, col=3)
text(5, 1.5, TeX("$P(NS | ND) = ?$"), cex=1.1, col=4)
```

In the tree diagram in Figure \@ref(fig:treeForSpam02), we can easily get the probabilities of having and not having the word "discount". This can be done using the Law of Total Probability (Section \@ref(totalProbability)):
\begin{equation*}
    P(D) = P(D|S) P(S) + P(D|NS) P(NS) = 0.7 \times 0.4 + 0.1 \times 0.6 = 0.34 .
\end{equation*}
Given that having and not having the word "discount" are two mutually exclusive events, $P(ND) = 1 - P(D) = 0.66$. Although this helps, we still do not know how to get to the answer of our question.

So, we should take a step back to the multiplication law from Section \@ref(probabilityMultiplication). We can notice that in the example we had, we can work out the final probability equally in any sequence: if we first have event B and then event A, or if we we have first A and then B. In our example with vampires, the order did not matter (the first student becomes first no matter what). But in general, this means that the probability of this joint outcome can be calculated in one of two possible ways:
\begin{equation}
    P(A \cap B) = P(B) \times P(A|B) = P(A) \times P(B|A) .
    (\#eq:ProbmultiplicationLawJoint)
\end{equation}

For the example at hand, this can be written as:
\begin{equation*}
...
1. $P(S) = 0.4$;
2. $P(D|S) = 0.7$;
3. $P(D) = 0.34$ - calculated earlier.

Inserting them in the formula \@ref(eq:BayesInExample) gives us the answer:
\begin{equation*}
...
The theorem's origins trace back to the work of the Reverend Thomas Bayes, whose work "An Essay Toward Solving a Problem in the Doctrine of Chances" was published posthumously in 1764 [@Bayes]. The derivation begins with the multiplication law for two events, A and B, which states that the probability of both events occurring together can be expressed in two ways (as we saw in equation \@ref(eq:ProbmultiplicationLawJoint)):
\begin{equation*}
    P(A \cap B) = P(B) \times P(A|B) = P(A) \times P(B|A) .
\end{equation*}
