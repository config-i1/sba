...
<!-- If we come back to the multiplication law from Section \@ref(probabilityMultiplication), we will notice that in our example, we can work out the final probability equally in any sequence: if we first have event B and then event A, we will have the same outcome as if we had first A and then B. In our example with vampires, the order does not matter (the first student becomes first no matter what). But in general, this means that the probability of this joint outcome can be calculated in one of two ways: -->
<!-- \begin{equation} -->
