...
*Mathematical* model is a model that is represented in a set of equations. It is more complex than the previous two, because it requires an understanding of mathematics. The mass-energy equivalence equation is an example of such model:
\begin{equation*}
    E = m c^2 .
\end{equation*}

Finally, the *imitation* model is a miniature reproduction of a real object or a process. This can be, for example, a physical model of a building standing in a room of an architect, or a mental arrangement in psychology.

In this textbook, we will deal with only first three types of models, focusing on the third one.



<!-- The word **analysis** itself means breaking a complex topic into smaller pieces in order to better understand it. We cannot  -->
...
1. Uncertainty about the data (see Section \@ref(uncertaintyData));
