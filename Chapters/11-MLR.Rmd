...
## Interpretation of parameters {#linearRegressionInterpretation}
...
## Prediction with regression {#linearRegressionPrediction}
One of the things we can do with regression is get an understanding of what to expect from the response variable given a set of explanatory variables. Consider a model that we estimated in Section \@ref(OLSMLR):
```{r eval=FALSE}
costsModel01 <- lm(overall~size+materials+projects+year, SBA_Chapter_11_Costs)
```
...
This question gives us all the necessary values of the variables, which we can now insert in the equation \@ref(eq:regressionOverall01) to get the answer. What we need to do is just drop $\epsilon_j$ (to switch to the regression line) and to substitute:
...
Note that because we do not have type in the model above, we cannot use this variable. We will discuss how to introduce it in the model in Chapter \@ref(dummyVariables). Inserting all the above, we get:
\begin{equation*}
    \widehat{overall}_j = 614.3227 + 1.3471 \times 90 + 0.8706 \times 190 - 1.5921 \times 3 - 0.1602 \times 2008 = 574.5178.
\end{equation*}
So, we can conclude that the **expected** (or **average**) overall costs for such project should be 574.5178. Note the boldface words "expected" and "average". The number does not tell you what you will definitely have, but instead only indicates the general tendency in this setting. You can think of it as a value that you would get as an arithmetic mean of many similar projects with exactly the same set of variables (defined in the question).
