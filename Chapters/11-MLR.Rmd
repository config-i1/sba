...
Here how the bi-variate relations between the variables in our dataset look like (Figure \@ref(fig:SBA_Chapter_11_CostsSpread)):

```{r SBA_Chapter_11_CostsSpread, fig.cap="Spread plot between variables in the building costs dataset."}
spread(SBA_Chapter_11_Costs)
```

While the plot in Figure \@ref(fig:SBA_Chapter_11_CostsSpread) gives a good idea about the relations, for example, between the size of property and the overall costs (it seems to be linear positive) or between material and the overall costs (linear positive again), it does not tell us much about the complex relation between one variable (overall costs) and several others.
...
\begin{equation*}
    \begin{aligned}
        ...
        \vdots
        & \sum_{j=1}^n y_j x_{k-1,j} - b_0 \sum_{j=1}^n x_{k-1,j} - b_1 \sum_{j=1}^n x_{1,j} x_{k-1,j} - b_2 \sum_{j=1}^n x^2_{2,j} - \dots - b_{k-1} \sum_{j=1}^n x^2_{k-1,j} = 0
    \end{aligned}
\end{equation*}
...
\begin{equation}
    \frac{\delta \mathrm{SSE}}{\delta \mathbf{b}} = - 2 \mathbf{X}' \mathbf{y} + 2 \mathbf{X}' \mathbf{X} \mathbf{b} = 0.
    (\#eq:MLROLS03)
\end{equation}
