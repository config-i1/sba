...
7. In the simple case we are discussing in this Chapter, the error term also has some fixed variance;
8. The parameters of the model should also be independent of the error term for the same reason (4).

In a general case, the true model can be represented as:
\begin{equation}
    y_j = \mu_{y,j} + \epsilon_j .
    (\#eq:TrueModel)
\end{equation}
where $\mu_j$ is the structure that we can capture in theory with the correct set of explanatory variables, and $\epsilon_j$ is the true error term that mathematically has $\mathrm{E}(\epsilon_j)=0$ due to (5) and $\mathrm{V}(\epsilon_j)=\sigma^2$ due to (7). The property (6) can be written mathematically as $\mathrm{E}(x_{i,j}\epsilon_j)=0$ for any $i$ and $j$.

For now, we assume that when we estimate a model, we know its form and have all the necessary variables. We will discuss what happens when this is violated in Chapter \@ref(assumptions).
