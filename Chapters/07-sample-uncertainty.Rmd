...
typeErrorsTable <- data.frame(C1 = c(rep("The data tells us",4)),
                              C2 = c(rep("Fail to reject H$_0$",2), rep("Reject H$_0$",2)),
...
colnames(typeErrorsTable) <- c("","","H$_0$ is true","H$_0$ is wrong")
...
**Type II** error (aka "false negative", i.e. we do not find effect, while we should have found it) happens when we fail to reject the wrong hypothesis (H$_0$ is not true). The probability of this event equals to $\beta$, which can be calculated based on the assumed distribution, the critical and calculated values for the hypothesis.
...
::: example
Just as a reminder, in a village, there lived a boy who one day decided to make a practical joke of his fellow villagers. He ran around the main street crying "Wolf!". We should acknowledge that there was no wolf at that stage, so in our terms we would say that the H$_0$: $\mu=0$ was true. But the villagers who have heard the boy went on the streets to help. They rejected the correct null hypothesis in order to help the boy, and they were surprised to find that there were no wolves on the streets. Thus the villagers made a **Type I error**.

Next week, the boy encountered a wolf on the main street and started crying "Wolf!", calling for help. Alas, this time nobody believed the boy and nobody came out to help, and thus the villagers rejected the correct null hypothesis in favour of the wrong one, H$_1$: $\mu\neq 0$. By doing so they have made the **Type II error**. If the villagers knew statistics, they would understand that failing to reject H$_0$ does not mean that it is true.
:::

Coming back to the table \@ref(tab:typeErrorsTable), there is a notion of "Power of the test", which is the probability to correctly reject the wrong null hypothesis ($1-\beta$). The power of the test is a useful theoretical idea, which allows selecting appropriate statistical tools in different circumstances and is discussed later in this section.

In real life, we never know whether the null hypothesis is true or not, but we could get a general idea about the performance of statistical tests in specific situation and decide, which of them to use, given their power. In general, the more information about the "true" parameters and model you can provide to the test, the more powerful it is. For example, if you can assume that the assumption of normality of distribution of mean holds, then you might want to choose between t-test and z-test. The latter is more powerful than the former, but is only applicable when you know the population standard deviation.

::: remark
As discussed in the Section \@ref(hypothesisTesting), when formulating the null hypothesis as equality, it is in fact almost always wrong, because it is close to impossible for a parameter to be equal to a specific value. In the light of this, the null hypothesis should be reformulated as:
\begin{equation*}
    \mathrm{H}_0: \mu \approx a, \mathrm{H}_1: \mu \neq a ,
\end{equation*}
instead of being the "equal to". In the formula above, $a$ is the value we are comparing our statistics with. Note that in case of one-tailed tests, this is no longer an issue, because the null hypothesis in that case compares the value with a set of values (e.g. H$_0$:$\mu\leq a$).
:::

Finally, sometimes analysts refer to the **Type 0** error. This is the error that arises when an answer is obtained to the wrong question. This does not have any mathematics behind it but is important in general: we need to understand what questions to ask and how to formulate them correctly before doing the test.
